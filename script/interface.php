	...
	foreach ($table as $table1) {
		if ($db->type == "pgsql") {
			...
			while ($tbl = $db->fetch_object($res)) {
				...
				} elseif ( strpos($tbl->Type, 'smallint') !== false || strpos($tbl->Type, 'double')!== false || strpos($tbl->Type, 'float') !== false ) {
				...
				} elseif ( strpos($tbl->Type, 'date') !== false ) {
					$sql_where.=' OR '.$table1.'.'.$fieldname." LIKE '".$db->escape($keyword)."%'";
				} else {
	...
	if ($nb_results == 0) {
		...
		while ($obj = $db->fetch_object($res)) {
			...
			if ($complete_label) {
				...
				elseif (empty($label) && !empty($o->name)) $label = $o->name;
				elseif (empty($label) && !empty($o->nom)) $label = $o->nom;
				elseif (empty($label) && !empty($o->title)) $label = $o->title;
				elseif (empty($label) && !empty($o->ref)) $label = $o->ref;
				elseif (empty($label) && !empty($o->id)) $label = $o->id;
			}
